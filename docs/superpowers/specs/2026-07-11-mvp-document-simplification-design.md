...
3. 删除 PRD/TDD 中鉴权、隐私、安全、内容治理、年龄、地区、法律、合规和运营防御章节及其交叉引用。
4. 简化数据模型、API、页面、Backlog、质量 Gate、风险表、Runbook、上线清单和开放问题。
5. 把实施顺序收口为：平台骨架 → 出生资料与时间 → 首个确定性排盘模块 → 报告与 Agent → 第二模块 → 日运。
6. 重写 README，使其与精简后的 PRD/TDD 一致，不再使用过时的 `iztro`、Token + password、先做 Agent 或一次覆盖全部术数的路线。
